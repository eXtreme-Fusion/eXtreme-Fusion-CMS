...
class gapi
{
	...
	*/
	public function requestAccountData($start_index=1, $max_results=20)
	{
		...
		{
			$this->Error=array(__('GAPI: Failed to request account data. Error: :error.', array(':error' => strip_tags($response['body']))));
		}
	}
	...
	*/
	public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
	{
		...
		if($start_date==null)
		{
			$start_date=HELP::showDate('shortdate',strtotime('1 month ago'));
		}
		...
		{
			$this->Error=array(__('GAPI: Failed to request report data. Error: :error.', array(':error' => ! empty($response['body']) ? strip_tags($response['body']) : __('There is no connection with Google Analytics server.'))));
		}
	}
	...
	*/
	protected function authenticateUser($email, $password)
	{
		...
		{
			$this->Error=array(__('GAPI: Failed to authenticate user. Error: :error.', array(':error' => strip_tags($response['body']))));
		}
		else
		{
	...
	*/
	protected function httpRequest($url, $get_variables=null, $post_variables=null, $headers=null)
	{
		...
		{
			$this->Error=array(__('Invalid http interface defined. No such interface: :interface.', array(':error' => gapi::http_interface)));
		}
	}
	...
	*/
	private function fopenRequest($url, $get_variables=null, $post_variables=null, $headers=null)
	{
		...
		return array('body'=>$response!==false?$response:__('Request failed, fopen provides no further information.'),'code'=>$response!==false?'200':'400');
	}
	...
	*/
	public function __call($name,$parameters)
	{
		...
		{
			$this->Error=array(__('No such function: :name.', array(':name' => $name)));
		}
		...
		$this->Error=array(__('No valid root parameter or aggregate metric called: :name.', array(':name' => $name)));
	}
}
...
class gapiAccountEntry
{
	...
	*/
	public function __call($name,$parameters)
	{
		...
		$this->Error=array(__('No valid property called: :name.', array(':name' => $name)));
	}
}
...
class gapiReportEntry
{
	...
	*/
	public function __call($name,$parameters)
	{
		...
		$this->Error=array(__('No valid metric or dimension called: :name.', array(':name' => $name)));
	}
}