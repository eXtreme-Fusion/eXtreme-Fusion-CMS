...
class DownloadSett {
	...
	{
		$this->_cache = $this->_system->cache('settings', NULL, 'download');
		if ($this->_cache === NULL)
		{
			$query = $this->_pdo->getData('SELECT * FROM [download_settings]');
			foreach($query as $data)
			{
				$this->_cache[$data['key']] = $data['value'];
			}

			$this->_system->cache('settings', $this->_cache, 'download');
		}
		return $this->_cache;
	}
	...
	{	
		$this->_system->clearCache('download', array('settings'));
	}
	 ...
	 * @uses    Database
	 */
	public function update(array $download_settings)
	{
		foreach ($download_settings as $key => $value)
		{
			if (isset($this->_cache[$key]))
			{
				...
				$count = $this->_pdo->exec('UPDATE [download_settings] SET `value` = :value WHERE `key` = :key', array(
