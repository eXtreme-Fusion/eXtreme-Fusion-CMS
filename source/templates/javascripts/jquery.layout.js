 ...
 */
	...
	,	defaults: { // default options for 'all panes' - will be overridden by 'per-pane settings'
		...
		,	togglerTip_open:		"Zwiń panel"		// Toggler tool-tip (title)
		,	togglerTip_closed:		"Otwórz panel"		// ditto
		,	resizerTip:				"Zmień rozmiar okna"	// Resizer tool-tip (title)
		,	sliderTip:				"Rozwiń" // resizer-bar triggers 'sliding' when pane is closed
	...
	,	togglers: {
		...
		,	cssDef: { // DEFAULT CSS - applied if: options.PANE.applyDefaultStyles=true
				background: "url(templates/images/ui/handle-icon.png) no-repeat 50% 50%"
			}
		}
	,	content: {
			cssReq: {
 ...
 */
	...
	*/
	function addToggleBtn (selector, pane) {
		...
		if ($E)
			$E
				.attr("title", state[pane].isClosed ? "Otwórz" : "Zwiń")
				.click(function (evt) {
					toggle(pane);
					evt.stopPropagation();
				})
